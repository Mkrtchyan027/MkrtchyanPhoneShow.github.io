...
import { allwishlist, deleteWishById, selectwishlist } from '../../features/wishlist/wishlistSlice';
...
export const WishList:React.FC=():JSX.Element=>{
        ...
        onAuthStateChanged(auth, (user) => {
            if (user && user.email) {
                dispatch(userFetchLink({ func: getAllDataByFields, obj: { collectionName: EnumFirestore.USERS, obj: { "email": user.email } } }))
                    ...
                    .then((e: any) => {
                        if (e) {
                            dispatch(userFetchLink({ func: getAllDataByFields, obj: { collectionName: EnumFirestore.WISHLIST, obj: { "userId": getUser(e[0]).payload.id } } }))
                                .unwrap()
                                .then((res: any) => {
    ...
    return <div className='wishlistdiv'><br/>
        <h2 className='h22'>Wishlist</h2>

        <div className='wishlistmain'>
            {
                wishlist?.arr?.map((e: any) => {
                    return <div className='imh' key={e.id}>
                        <h3>{e.product.name}</h3>
                        <h5>{e.product.brand}</h5>
                        <img src={e.product.picurl[0]} style={{width:"300px"}} />
                        ...
                        <span>
                            <svg onClick={() => {
                                dispatch(userFetchLink({ func: deleteDataById, obj: { collectionName: EnumFirestore.WISHLIST, id: e.id } }))
                                .then(i=>{
                                    dispatch(deleteWishById(e.id))
                                })
                            ...
                            <Link className='productbutton' to={'/seeproduct/' + e.product.id}>See Product </Link>
                        </span>
                    </div>
                })
            }
